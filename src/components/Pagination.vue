...
<div class="pagination__list">
    <div class="catalog__sort">
        ...
        <div class="catalog__sort-button" :class="activeSortClass('new')">
            <a href="#" @click.prevent="sortParam='new'" class="catalog__sort-link">		Недавно добавленные		</a>
        </div>
        <div class="catalog__sort-button" :class="activeSortClass('price|expensive')">
            <a href="#" @click.prevent="sortParam='price|expensive'" class="catalog__sort-link">		Сначала дорогие		</a>
        </div>
        <div class="catalog__sort-button" :class="activeSortClass('price|cheap')">
            <a href="#" @click.prevent="sortParam='price|cheap'" class="catalog__sort-link">		Сначала дешевые		</a>
        </div>
    </div>
    ...
    <div class="load__more" v-show="loadMoreHandler()">
        <a href="#" @click.prevent="loadMore()" class="load__more-link">		ПОКАЗАТЬ ЕЩЕ		</a>
    </div>
    <div class="products__nav" v-show="loadMoreHandler()">
        <ul class="products__nav-list">
            <li class="products__nav-item" :class="activeClass(i)" v-for="i in pageCount" :key="i">
                <a href="#" @click.prevent="desiredPage(i)" class="products__nav-link">		<span>{{ i }}</span>		</a>
            </li>
        </ul>
    </div>
</div>
